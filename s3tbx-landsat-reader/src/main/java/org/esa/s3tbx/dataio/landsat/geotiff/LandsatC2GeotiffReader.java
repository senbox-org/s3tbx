 ...
 */
public class LandsatC2GeotiffReader extends AbstractProductReader {
    ...
    private LandsatC2Metadata landsatC2Metadata;
    private LandsatQA landsatQA;
    private List<Product> bandProducts;
    private VirtualDir virtualDir;
    private String basePath;

    public LandsatC2GeotiffReader(ProductReaderPlugIn readerPlugin) {
        this(readerPlugin, Resolution.DEFAULT);
    }

    public LandsatC2GeotiffReader(ProductReaderPlugIn readerPlugin, Resolution targetResolution) {
        super(readerPlugin);
    ...
    protected Product readProductNodesImpl() throws IOException {
        ...
        landsatC2Metadata = LandsatC2MetadataFactory.create(mtlFile);
        landsatQA = LandsatQAFactory.create(mtlFile);
        // todo - retrieving the product dimension needs a revision
        Dimension productDim;
        switch (targetResolution) {
            case L8_REFLECTIVE:
                productDim = landsatC2Metadata.getReflectanceDim();
                if (productDim == null) {
                    productDim = landsatC2Metadata.getThermalDim();
                }
                break;
            case L8_PANCHROMATIC:
                productDim = landsatC2Metadata.getPanchromaticDim();
                break;
            default:
                productDim = landsatC2Metadata.getPanchromaticDim();
                if (productDim == null) {
        ...
        MetadataElement metadataElement = landsatC2Metadata.getMetaDataElementRoot();
        Product product = new Product(getProductName(mtlFile), landsatC2Metadata.getProductType(), productDim.width, productDim.height);
        product.setFileLocation(mtlFile);
        product.getMetadataRoot().addElement(metadataElement);

        ProductData.UTC utcCenter = landsatC2Metadata.getCenterTime();
    ...
    private void addBands(Product product) throws IOException {
        ...
        MetadataElement metadata = landsatC2Metadata.getProductContents();
        metadata.addElement(landsatC2Metadata.getProjectionAttributes());
        final MetadataAttribute[] productAttributes = metadata.getAttributes();
        final Pattern pattern = landsatC2Metadata.getOpticalBandFileNamePattern();
        ...
        for (MetadataAttribute metadataAttribute : productAttributes) {
            ...
            if (matcher.matches()) {
                ...
                if (bandProduct != null) {
                    ...
                    String bandName = landsatC2Metadata.getBandNamePrefix(bandNumber);
                    Band band = addBandToProduct(bandName, srcBand, product);
                    band.setScalingFactor(landsatC2Metadata.getScalingFactor(bandNumber));
                    band.setScalingOffset(landsatC2Metadata.getScalingOffset(bandNumber));

                    band.setNoDataValue(0.0);
                    band.setNoDataValueUsed(true);

                    band.setSpectralWavelength(landsatC2Metadata.getWavelength(bandNumber));
                    band.setSpectralBandwidth(landsatC2Metadata.getBandwidth(bandNumber));

                    band.setDescription(landsatC2Metadata.getBandDescription(bandNumber));
                    ...
                    if (readAs != null) {
                        switch (readAs.toLowerCase()) {
                            case READ_AS_REFLECTANCE:
                                band.setDescription(landsatC2Metadata.getBandDescription(bandNumber) + " , as TOA Reflectance");
            ...
            } else if (attributeName.equals(landsatC2Metadata.getQualityBandNameKey()) && landsatQA != null) {
                String fileName = metadataAttribute.getData().getElemString();
                File bandFile = virtualDir.getFile(basePath + fileName);
                ProductReader productReader = plugIn.createReaderInstance();
                Product bandProduct = productReader.readProductNodes(bandFile, null);
                if (bandProduct != null) {
                    ...
                    if (Resolution.DEFAULT.equals(targetResolution)) {
                        Dimension dimension = landsatC2Metadata.getReflectanceDim();
                        if (dimension == null) {
                            dimension = landsatC2Metadata.getThermalDim();
                        }
    ...
    private Dimension getBandDimension(Band srcBand, Resolution targetResolution) {
        switch (targetResolution) {
            case L8_REFLECTIVE:
                return landsatC2Metadata.getReflectanceDim();
            case L8_PANCHROMATIC:
                return landsatC2Metadata.getPanchromaticDim();
            default:
