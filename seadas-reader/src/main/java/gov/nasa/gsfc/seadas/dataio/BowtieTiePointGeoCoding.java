...
//            start = findStart(latGrid.getRasterWidth() - 1);
        ...
        if(start == -1) {       // did not find an overlap
            ...
            scanlineOffset = start % scanlineHeight;
        }
    }
    ...
    private int findStart(int x) {
        ...
        for (int i = 1; i < latGrid.getGridHeight(); i++) {
            ...
            if (increaseDecreaseCount > 1 && change == -1) {
                return (int) (i * latGrid.getSubSamplingY());
    ...
    public boolean transferGeoCoding(final Scene srcScene, final Scene destScene, final ProductSubsetDef subsetDef) {
        ...
        if (subsetDef == null || subsetDef.getRegion() == null) {
            final TiePointGrid clonedLatGrid = latGrid.cloneTiePointGrid();
            final TiePointGrid clonedLonGrid = lonGrid.cloneTiePointGrid();
            destProduct.addTiePointGrid(clonedLatGrid);
            destProduct.addTiePointGrid(clonedLonGrid);
            BowtieTiePointGeoCoding destGeo = new BowtieTiePointGeoCoding(clonedLatGrid, clonedLonGrid, scanlineHeight);
